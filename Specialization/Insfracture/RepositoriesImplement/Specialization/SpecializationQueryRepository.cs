...
namespace Specialization.Insfracture.RepositoriesImplement.Specialization
{
    public class SpecializationQueryRepository : ISpecializationQueryRepository
    {
        ...
        // Get Filtered Specializations (used by doctors)
        public async Task<Response<List<Entities.EntityClass.Specialization>>> GetFiltered()
        {
            var response = new Response<List<Entities.EntityClass.Specialization>>();
            try
            {
                // Get specializations that are used in DoctorSpecialization table
                var result = await _dataAccess.LoadDataUsingProcedure<Entities.EntityClass.Specialization, dynamic>("Specialization_GetFiltered", new
                {
                });
                response.Result = result.ToList();
                response.IsSuccess = true;
