...
{
    public class RolePermissionCommandRepository : IRolePermissionCommandRepository
    {
        private readonly ISqlDataAccessLayer _dataAccess;
        public RolePermissionCommandRepository(ISqlDataAccessLayer dataAccess)
        {
        ...
        public async Task<Response<bool>> Delete(int id)
        {
            ...
            try
            {
                var result = await _dataAccess.LoadSingleDataUsingProcedure<Entities.EntityClass.RolePermission, dynamic>("RolePermission_DeleteById", new
                {
                    Id = id
                });
        ...
        public async Task<Response<int>> Insert(Entities.EntityClass.RolePermission entity)
        {
            var response = new Response<int>();
            try
            {
                var result = await _dataAccess.SaveDataUsingProcedureReturnIdWithIntDataType<Entities.EntityClass.RolePermission>("RolePermission_Insert", entity);
                if (result == 0)
                {
        ...
        public async Task<Response<int>> Update(Entities.EntityClass.RolePermission entity)
        {
            var response = new Response<int>();
            try
            {
                var result = await _dataAccess.SaveDataUsingProcedureReturnIdWithIntDataType<Entities.EntityClass.RolePermission>("RolePermission_Update", entity);
                if (result == 0)
                {
