...
namespace AuthenticationSystem.Insfrastructure.RepositoriesImplement.Role
{
    public class RoleCommandRepository : IRoleCommandRepository
    {
        ...
        public RoleCommandRepository(ISqlDataAccessLayer dataAccess)
        {
        ...
        public async Task<Response<bool>> Delete(int id)
        {
            ...
            try
            {
                var result = await _dataAccess.LoadSingleDataUsingProcedure<Entities.EntityClass.Role, dynamic>("Role_DeleteById", new
                {
                    Id = id
                });
        ...
        public async Task<Response<int>> Insert(Entities.EntityClass.Role entity)
        {
            var response = new Response<int>();
            try
            {
                var result = await _dataAccess.SaveDataUsingProcedureReturnIdWithIntDataType<Entities.EntityClass.Role>("Role_Insert", entity);
                if (result == 0)
                {
        ...
        public async Task<Response<int>> Update(Entities.EntityClass.Role entity)
        {
            var response = new Response<int>();
            try
            {
                var result = await _dataAccess.SaveDataUsingProcedureReturnIdWithIntDataType<Entities.EntityClass.Role>("Role_Update", entity);
                if (result == 0)
                {
