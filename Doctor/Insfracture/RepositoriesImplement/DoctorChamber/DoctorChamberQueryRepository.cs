...
namespace Doctor.Insfracture.RepositoriesImplement.DoctorChamber
{
    public class DoctorChamberQueryRepository : IDoctorChamberQueryRepository
    {
        ...
        public async Task<ApiResponse<List<Entities.CountryEntity.Division>>> GetAllDivision()
        {
            ...
            try
            {
                ...
                ApiResponseHelper.SetSuccessResponse(
                    apiResponse,
                    apiResponse.Results,
                    "All divisions retrieved successfully",
                    StatusResponseMessage.success,
                    StatusCodes.Status200OK
                );
            }
        ...
        public async Task<Response<List<Entities.EntityClass.DoctorEntity.DoctorChamber>>> GetByDoctorId(int doctorId)
        {
            ...
            try
            {
                var result = await _dataAccess.LoadDataUsingProcedure<Entities.EntityClass.DoctorEntity.DoctorChamber, dynamic>("DoctorChamber_GetByDoctorId", new
                {
                    DoctorID = doctorId
                });
                response.Result = result.ToList();
